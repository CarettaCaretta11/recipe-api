...
class PublicUserApiTests(TestCase):
    ...
    def test_user_with_email_exists_error(self):
        """ Test that an error is returned if a user with the email exists."""
        payload = {
            'email': 'test@example.com',
    ...
    def test_password_too_short_error(self):
        """ Test that an error is returned if the password is shorter than 5 chars. """
        payload = {
            'email': 'test@example.com',
            'password': 'pw',
            'name': 'Test User',
    ...
    def test_create_token_for_user(self):
        """ Test that a token is generated given valid credentials. """
        user_details = {
            'name': 'Test User',
            'email': 'test@example.com',
            'password': 'samplepass123',
    ...
    def test_create_token_bad_credentials(self):
        ...
        create_user(email='test@example.com', password='samplepass321')

        payload = {'email': 'test@example.com', 'password': 'invalidpass123'}
        res = self.client.post(TOKEN_URL, payload)
